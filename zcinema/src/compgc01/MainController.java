...
public class MainController extends SceneCreator {
	...
	static String sceneToLoad = "UserScene.fxml";
...
//		 }
	...
	public void saveClick(ActionEvent event) throws IOException {
	    ...
	    if (!updateFirstName.getText().trim().isEmpty()) {
	        Main.modifyJSONUserFile(currentUser, "firstName", updateFirstName.getText());
	        currentUser.setFirstName(updateFirstName.getText());
	    }
	    if (!updateLastName.getText().trim().isEmpty()) {
            Main.modifyJSONUserFile(currentUser, "lastName", updateLastName.getText());
            currentUser.setLastName(updateLastName.getText());
	    }
	    if (!updateEmail.getText().trim().isEmpty()) {
            Main.modifyJSONUserFile(currentUser, "email", updateEmail.getText());
	...
	private void changeImage() throws IOException {
		...
		try {
			...
			// code used to write to .txt file
			// keep here just in case we need it at some point
			/*
			 * Path p = Paths.get(infile.toURI()); String writeToPath = "";
			 * 
			 * if (employeeMode) { writeToPath = "./employees.txt"; } else {
			 * writeToPath = "./customers.txt"; }
			 * 
			 * try (BufferedWriter writer = new BufferedWriter(new
			 * FileWriter(writeToPath, true)); PrintWriter out = new
			 * PrintWriter(writer)) { out.print("," + p.toString()); } catch
			 * (IOException ex) { //
